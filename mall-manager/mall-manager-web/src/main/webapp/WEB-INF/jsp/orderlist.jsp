        ...
        layui.use(['layer', 'table','form','upload','laydate','element'], function ()
        {
            ...
            { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                { //编辑
					...
					$.ajax({
						type : "post",
						url : "http://"+OrderIp()+":8894/order/getOrderByOrderId",
						async: false,
						...
						success : function(resp) {
								str = str+ "<div class='layui-collapse ' lay-accordion=''>";
									...
									function acc(item) {
										if (item.status==0) {
											return "<p style='color:#5FB878; font-size:15px;'>正常交易</p>";
										}else if(item.status==1){
											return "<p style='color:#FF5722; font-size:15px;'>正在退款</p>";
										}else if(item.status==2) {
											return "<p style='color:#01AAED; font-size:15px;'>退款成功</p>";
										}else{
											return "<p style='color:#01AAED; font-size:15px;'>未定义</p>";
										}
									}
									function add(ac) {
										return "<p style='color:#FF5722; font-size:15px;'>"+ac.title+"<p>";
									}
									function abb (ab) {
										return "<p style='color:#01AAED; font-size:14px;'>"+ab.price/100+"元"+"<p>";
									}
									function aee (ae) {
										return "<p style='color:#2F4056; font-size:15px;'>"+ae.num+"台"+"<p>";
									}
									function agg (ag) {
					...
					if(status==1||status==6||status==8){
						layer.confirm('是否删除该订单？', {
							  ...
							  ,btn1: function(index, layero){
								  $.ajax({
									   type: "POST",
									   url: "http://"+OrderIp()+":8894/order/deleteOrder",
									   data: "orderId="+data.orderId,
